...
import { appSettings, installations, userInstallations, webhookLogs } from '$lib/server/db/schema';
import {
	autoLinkPersonalInstallations,
...
export const getDashboardOverview = query(async () => {
	...
	await autoLinkPersonalInstallations(currentUser.id, currentUser.username);
	...
	const rows = await db
		...
		.where(eq(userInstallations.userId, currentUser.id))
